...
{
    public class SCMInstruction : Instr
    {
        private Dictionary<string, ID> _ids;
        private Dictionary<Point, Literal> _literals;
        private Dictionary<string, QGate> _gates;

        public SCMInstruction(Dictionary<string, ID> ids,Dictionary<Point, Literal> literals, Dictionary<string, QGate> gates) : base()
        {
            _ids = ids;
            _literals = literals;
            _gates = gates;
            ...
            if(args.Count == 2)
            {
                ...
                if(_gates.ContainsKey(nameGate))
                {
                    if (!_ids[nameGate].IsConst)
                    {
                        ...
                        if (countStates >= 0)
                        {
                            _gates[nameGate].SetCountStates(countStates);
                        }
                        else
                        {
                            //error
