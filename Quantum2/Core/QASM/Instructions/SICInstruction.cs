...
{
    public class SICInstruction : Instr
    {
        private Dictionary<string, ID> _ids;

        private Dictionary<Point, Literal> _literals;

        public SICInstruction(Dictionary<string, ID> ids, Dictionary<Point, Literal> literals) : base()
        {
            _ids = ids;
            _literals = literals;
        ...
        public override void ExecuteInstruction(List<ASTNode> args)
        {
            if (args.Count == 2)
            {
                ...
                double imag = Convert.ToDouble(GetValue(args[1].Token, _ids, _literals));

                _ids[args[0].Token.NameID].Value = new Complex(complex.Real, imag);
            }
            else
            {
                //error
