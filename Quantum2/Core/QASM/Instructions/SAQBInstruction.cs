...
{
    public class SAQBInstruction : Instr
    {
        private Dictionary<string, ID> _ids;

        private Dictionary<Point, Literal> _literals;

        public SAQBInstruction(Dictionary<string, ID> ids, Dictionary<Point, Literal> literals) : base()
        {
            _ids = ids;
            _literals = literals;
        ...
        public override void ExecuteInstruction(List<ASTNode> args)
        {
            if(args.Count == 3)
            {
                ...
                Complex alpha = (Complex)GetValue(args[1].Token, _ids, _literals);

                double realBeta = Convert.ToDouble(GetValue(args[2].Token, _ids, _literals));

                qubit.SetACBR(alpha, realBeta);
            }
            else
            {
                //error
