...
{
    public class SHRIInstruction : Instr
    {
        private Dictionary<string, ID> _ids;

        private Dictionary<Point, Literal> _literals;

        public SHRIInstruction(Dictionary<string, ID> ids, Dictionary<Point, Literal> literals) : base()
        {
            _ids = ids;
            _literals = literals;
        ...
        public override void ExecuteInstruction(List<ASTNode> args)
        {
            if (args.Count == 2)
            {
                ...
                int op1 = Convert.ToInt32(GetValue(args[1].Token, _ids, _literals));
