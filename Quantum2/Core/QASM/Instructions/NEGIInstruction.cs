...
{
    public class NEGIInstruction : Instr
    {
        private Dictionary<string, ID> _ids;

        private Dictionary<Point, Literal> _literals;

        public NEGIInstruction(Dictionary<string, ID> ids, Dictionary<Point, Literal> literals)
        {
            _ids = ids;
            _literals = literals;
        ...
        public override void ExecuteInstruction(List<ASTNode> args)
        {
            if (args.Count == 1)
            {
                ...
                object result = CIT(typeFirstOperand, op0 * -1);
